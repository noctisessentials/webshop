...
export function AcaciaLandingClient({ product, upsellProducts }: AcaciaLandingClientProps) {
  ...
  const handleAddToCart = async () => {
    ...
    for (const upsell of selectedUpsells) {
      ...
      if (selectedOption) {
        const syntheticColor: ProductColor = {
          name: selectedOption.label,
          slug: selectedOption.handle.split('-').pop() ?? selectedOption.label.toLowerCase(),
          hex: selectedOption.hex,
          inStock: true,
          wcId: selectedOption.wcId,
          wcSlug: selectedOption.handle,
        }
        addItem({ ...upsell, images: [{ src: selectedOption.image, alt: selectedOption.label }] }, syntheticColor, 1)
      } else {
              ...
              {upsellProducts.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-sm font-sans font-semibold text-dark flex-1">Maak je keuken compleet en bespaar 10%</h3>
                    <span className="inline-flex flex-shrink-0 rounded-full bg-[#CDEBFF] px-2.5 py-1 text-xs font-sans font-semibold text-dark">
                      Bundle deal
                    </span>
                  </div>
                  {upsellProducts.map((upsell) => {
                    ...
                    const activeLabel = activeOption
                      ? `${upsell.handle.includes('kitchenware') ? '19-delige keukenset' : 'Peper- en zoutmolens'} ${activeOption.label}`
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-sans font-semibold text-dark">{activeLabel}</p>
                            {colorOptions && (
